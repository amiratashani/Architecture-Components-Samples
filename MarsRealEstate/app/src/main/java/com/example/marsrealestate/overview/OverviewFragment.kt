...
class OverviewFragment : Fragment() {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        viewModel.updateFilter(
            when (item.itemId) {
                R.id.show_rent_menu -> hashMapOf("filter" to MarsApiFilter.SHOW_RENT.value)
                R.id.show_buy_menu -> hashMapOf("filter" to MarsApiFilter.SHOW_BUY.value)
                else -> hashMapOf("filter" to MarsApiFilter.SHOW_ALL.value)
            }
        )
        return true
    }
